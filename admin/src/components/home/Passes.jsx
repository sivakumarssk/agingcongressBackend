...
function Passes() {
    ...
    const fetchPasses = async () => {
        try {
            const response = await axios.get("https://admin.agingcongress.org/api/");
    ...
    const handleAddOrUpdateBenefit = async (pass) => {
        ...
        if (updatedPasses.length === 0) {
            ...
            try {
                await axios.patch(`https://admin.agingcongress.org/api/passes`, {
    ...
    const handleDeleteBenefit = async (passType, passIndex, benefitIndex) => {
        try {
            const response = await axios.delete(
                `https://admin.agingcongress.org/api/passes/${passType}/${benefitIndex}`
            );
    ...
    const handleSubmit = async () => {
        try {
            await axios.patch("https://admin.agingcongress.org/api/passes", passesForm, {
